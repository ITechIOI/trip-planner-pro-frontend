...
const buildInitialFormState = (
  editItem: ItineraryResponse | null | undefined,
  defaultDate: string,
): ItineraryFormFields => {
  if (!editItem) {
    return emptyFormState(defaultDate);
  }
...
export const ItineraryForm = ({
  ...
  const [formData, setFormData] = useState<ItineraryFormFields>(() =>
    buildInitialFormState(editItem, defaultDate),
  );
  ...
  const validate = () => {
    ...
    } else if (!isLocalDate(formData.date)) {
      newErrors.date = "Date must be valid";
    }

    if (!formData.startTime) {
      newErrors.startTime = "Start time is required";
    } else if (!isLocalTime(formData.startTime)) {
      newErrors.startTime = "Start time must be valid";
    }

    if (formData.endTime) {
      if (!isLocalTime(formData.endTime)) {
        newErrors.endTime = "End time must be valid";
      } else if (
        formData.startTime &&
        isLocalTime(formData.startTime) &&
  ...
  const handleConfirmEdit = async () => {
    ...
    try {
      ...
      setSubmitError(
        getItinerariesErrorMessage(error as UpdateItineraryMutationError),
      );
      setConfirmOpen(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      <ConfirmActionDialog
        ...
        description={`Save changes to "${editItem?.activityTitle ?? "this activity"}"?`}
        confirmLabel="Save"
